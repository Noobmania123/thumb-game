...
A pseudo-3D **2D racing game** made in Python + Pygame, designed to run from a USB drive and package to a standalone Windows `.exe` or Apple Silicon macOS `.app`.
...
- Apple Silicon macOS `.app` build script in Python (`build_macos.py`).
...
Use a native Apple Silicon Python 3.11+ install, then run:
...
If macOS blocks the app the first time, right-click `ThumbDriveRacer.app`, choose **Open**, then confirm.
